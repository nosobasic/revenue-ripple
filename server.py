...
_allowed_from_env = [s.strip() for s in (os.getenv("ALLOWED_ORIGINS", "").split(",")) if s.strip()]
_default_allowed = [
	"http://localhost:5173",
	"http://localhost:3000",
	"https://revenueripple.org",
	"https://www.revenueripple.org",
	"https://revenue-ripple.vercel.app"
]
# Regex for vercel preview domains
_vercel_regex = r"https://.*\\.vercel\\.app"

# Combine explicit allowlist and regex patterns (flask-cors accepts regex strings)
allowed_cors_origins = list({_vercel_regex, *_default_allowed, *_allowed_from_env})
...
# Ensure OPTIONS is handled early and cleanly for any path
@app.before_request
def handle_global_options():
    if request.method == 'OPTIONS':
        resp = make_response('', 204)
        # flask-cors will still decorate the response with appropriate headers
        return resp
    return None
