  ...
  const sendMessage = async () => {
    ...
    try {
      const res = await fetch(
        "https://revenue-ripple.onrender.com/api/ai-assistant",
        {
          method: "POST",
          headers: {
      ...
      setMessages((msgs) => [
        ...msgs,
        { from: "ai", text: data.reply || "Sorry, I couldn't process that." },
      ]);
    } catch {
      setMessages((msgs) => [
        ...msgs,
        {
          from: "ai",
          text: "Sorry, there was a problem connecting to the AI.",
        },
      ]);
    }
    setInput("");
    ...
    <div>
      ...
      {open && (
        <div
          ...
          style={{
            background: "white",
            boxShadow: "0 8px 32px 0 rgba(30, 58, 138, 0.25)",
          }}
        >
          <div
          ...
          >
            ...
            Ripple AI Assistant
            <button
              className="ml-auto text-white text-2xl leading-none"
          ...
          >
            {messages.map((msg, i) => (
              <div
                key={i}
                className={
                  msg.from === "user" ? "text-right mb-2" : "text-left mb-2"
                }
              >
                <span
                  className={
            ...
            {loading && (
              <div className="text-gray-400">Ripple is thinking...</div>
            )}
          </div>
