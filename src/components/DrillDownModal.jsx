...
export function DrillDownModal({
  ...
  const { data: drillDownData, isLoading } = useQuery({
    queryKey: [
      `/api/agents/${agentId}/drill-down/${metric}`,
      selectedTimeRange,
    ],
    enabled: isOpen,
  ...
  const generateContextualInsights = (data) => {
    ...
    if (data.change && Math.abs(data.change) > 10) {
      insights.push({
        ...
        description: `${metric} has ${
          data.change > 0 ? "increased" : "decreased"
        } by ${Math.abs(data.change).toFixed(1)}% in the selected period`,
        confidence: 95,
        actionable: true,
        recommendation:
          data.change > 0
    ...
    const highCorrelation = data.breakdown.correlations.find(
    ...
    if (highCorrelation) {
      insights.push({
        ...
        description: `${metric} shows ${Math.abs(
          highCorrelation.correlation * 100
        ).toFixed(0)}% correlation with ${highCorrelation.metric}`,
        confidence: 92,
        actionable: true,
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-6xl max-h-[90vh] overflow-hidden">
        <DialogHeader>
          ...
          <DialogDescription>
            Deep dive into {metric} with contextual insights and actionable
            recommendations
          </DialogDescription>
        </DialogHeader>
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <div className="text-right">
                            <div className="flex items-center space-x-1">
                              ...
                              >
                                {related.change > 0 ? "+" : ""}
                                {related.change.toFixed(1)}%
                              </span>
                              {getChangeIcon(related.change)}
                            </div>
                            <Badge
                              variant={
                                related.relation === "positive"
                                  ? "default"
                                  : "secondary"
                              }
                              className="text-xs"
                      ...
                      >
                        <div className="flex items-center justify-between">
                          <div>
                            ...
                            <p className="text-sm text-muted-foreground">
                              {Math.abs(correlation.correlation * 100).toFixed(
                                0
                              )}
                              % correlation
                            </p>
